...
function handleBoxClick(e) {
    ...
    boxes.forEach(box => {
        ...
        tiles.forEach(tile => {
            if (parseInt(tile.id) >= rangeMin && parseInt(tile.id) < rangeMax && tile.innerHTML !== "") {
                ...
                if (arr[number] !== 1) {
                    ...
                    // Ustawiony tutaj backgroundColor nadpisuje ten z pliku css,
                    // Inną opcją jest ustawienie w css background-color: rgb(0, 128, 255) !important;
                    // Ale !important lepiej unikać, dlatego nadajemy klasę .error

                    // tile.style.backgroundColor = "rgba(255,0,0,0.7)";
        ...
        if (counter === 9) {
            ...
            box.style.backgroundColor = "rgba(26, 255, 0, 0.3)";
        } else {
            box.style.backgroundColor = "rgb(0, 0, 255)";
        }
    });

    if (completedBoxesCounter === 9) { // przeniesione do runCompleted()
...
const runCompleted = () => {
    ...
    // Uruchamiamy animację w odstępie czasowym dla każdego kafla
...
window.onload = handleBoxClick; // window.onload = functionReference; !!!!
// Do poczytania: https://stackoverflow.com/a/18369286/9590769 ,
